...
export default function ContactIcons({
  ...
  return (
    <div className="no-scrollbar fixed right-10 bottom-5 z-50 flex flex-col gap-4 sm:right-3 sm:gap-6">
      <div
        className={`${hideIcon ? "sm:invisible sm:h-0 sm:opacity-0" : ""} relative transition-all sm:duration-300`}
      >
        <button
          className="bg-maincolor-500 flex h-12 w-12 items-center justify-center rounded-xl font-bold transition-all duration-200 hover:animate-none sm:h-9 sm:w-9 sm:text-sm md:animate-[scaleUpDown_5s_infinite] md:hover:scale-125 md:hover:drop-shadow-[0_0_0.5em_rgb(255,255,255,0.5)] lg:animate-[scaleUpDown_5s_infinite] lg:hover:scale-125 lg:hover:drop-shadow-[0_0_0.5em_rgb(255,255,255,0.5)]"
          onClick={() => openShowContact()}
        >
          <span className="bg-maincolor-900 rounded-full p-1.5 sm:bg-transparent sm:p-0">
            연락
          </span>
        </button>
        <div
          ...
          } absolute top-1/2 right-24 flex h-fit min-h-fit w-fit -translate-y-1/2 flex-col gap-2 rounded-xl bg-gray-500 p-4 font-semibold text-white transition-all duration-300 sm:right-12 sm:w-[75vw] sm:gap-6`}
        >
      ...
      <div
        className={`${hideIcon ? "sm:h-0" : ""} relative flex flex-col items-center gap-4 transition-[height] sm:gap-6 sm:overflow-x-visible sm:overflow-y-hidden sm:rounded-b-2xl sm:duration-300`}
      >
        <a
        ...
        <div className="mt-2 h-28 w-[2px] bg-linear-to-b from-white from-40% sm:hidden"></div>
      </div>
